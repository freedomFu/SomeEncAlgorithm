...
public class SM2 {
    ...
    private static BigInteger xg = new BigInteger("32C4AE2C"+"1F198119"+"5F990446"+"6A39C994"+"8FE30BBF"+"F2660BE1"+"715A4589"+"334C74C7",16);
    private static BigInteger yg = new BigInteger("BC3736A2"+"F4F6779C"+"59BDCEE3"+"6B692153"+"D0A9877C"+"C62A4740"+"02DF32E5"+"2139F0A0",16);

    private static ECDomainParameters ecc_bc_spec;
    private static int w = (int)Math.ceil(n.bitLength()*1.0/2)-1;
    private static BigInteger _2w = new BigInteger("2").pow(2);
     ...
     * @return
     */
    private boolean byteAllZero(byte[] buffer){
    ...
    private boolean checkPublicKey(ECPoint publicKey){
        if(!publicKey.isInfinity()){
            ...
            if(isBetween(x, new BigInteger("0"),p) && isBetween(y, new BigInteger("0"),p)){
    ...
    private static byte[] ZA(String IDA, ECPoint aPublicKey){
        ...
        byte[] ZA = sm3hash(ENTLA, idaBytes, a.toByteArray(), b.toByteArray(),xg.toByteArray(), yg.toByteArray(), aPublicKey.getXCoord().toBigInteger().toByteArray(),aPublicKey.getYCoord().toBigInteger().toByteArray());
        return ZA;
    }
    ...
    public boolean verify(String M, Signature signature, String IDA, ECPoint aPublicKey){
        if(!isBetween(signature.r, BigInteger.ONE, n)){
            return false;
        }

        if(!isBetween(signature.s, BigInteger.ONE,n)){
            return false;
        }
...
//        sm02.exportPrivateKey(privateKey, "privatekey.pem");
    ...
    private static class SM2KeyPair {
        ...
        private SM2KeyPair(ECPoint publicKey, BigInteger privateKey) {
    ...
    private static class TransportEntity implements Serializable{
        ...
        private TransportEntity(byte[] r, byte[] s, byte[] z, ECPoint pKey) {
            R = r;
            S = s;
            Z = z;
